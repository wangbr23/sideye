...
## 2026-09-12 — fixed `/sideye` timeout caused by leaked test servers

The reported `http://localhost:4096` timeout was a real hung OpenCode service, not a Sideye review-server failure. PID 4130 was an orphaned `opencode serve --hostname=127.0.0.1 --port=0` rooted in a deleted `sideye-cli-*` test fixture, had run for over a day at approximately 100% CPU, accepted connections on port 4096, and never answered `/global/health`. Seven more orphan servers from repeated CLI test runs had the same fixture provenance.

Root cause: `test/cli.test.ts` cleaned up the spawned Sideye CLI processes with `SIGKILL`. That bypassed the CLI's `SIGTERM` handler, so its child OpenCode servers survived with PPID 1. Cleanup now sends `SIGTERM`, waits up to five seconds for the launcher and its normal child cleanup, and uses `SIGKILL` only as a bounded fallback. Removed only the eight confirmed `sideye-cli-*` orphan processes. Verification: focused CLI suite 4 pass; full suite 123 pass; `bun run typecheck` and `git diff --check` clean; no matching headless OpenCode process remained after either test run.

## 2026-09-12 — T28 flow designed: plan revisions and queued feedback

Resolved LLD §11.6 before implementation. Diff rounds remain frozen code snapshots; rejecting or discussing a plan creates another immutable plan version inside the same round's submission cycle, never another diff round and never edit authorization. A revision carries the prior work package, prior plan, cumulative reviewer responses, and comments added since that candidate. Only an exact ready plan version can be approved.

The reviewer may intentionally approve a plan even when newer comments exist; the UI must state how many comments are not covered. Those comments, and any comments added while the fix runs, are never injected into in-flight work. They remain queued until status + consent capture the next diff round, where a fresh submission cycle picks up work absent from every approved payload. Failed revisions preserve the earlier ready plan as an approval option. The data model, API, flow, frontend states, failure paths, and test mapping were added to `docs/designs/2026-09-10-sideye-code-review-mvp-lld.md`; the load-bearing choice was appended to `docs/decisions.md`. T28 remains open pending implementation.

## 2026-09-12 — T28 implemented: immutable cycles and plan candidates

Added `src/server/submissions.ts` as the cycle/version authority. Candidate payloads are cloned, approved source identities are excluded from later cycles, lessons are limited to the candidate's comments, revisions snapshot queued comments, retries retain their version, and approvals require exact cycle/version identity. Plan prompts now include prior plans and cumulative feedback; semantic request-ID coverage is repaired once before a candidate fails. Fixes consume only the approved candidate.

The API exposes versioned plan events and revision/retry/approval identifiers. The action bar now shows version history, revision feedback, queued-comment approval wording, and permits submission after a consent-captured round. Focused submission-cycle tests pass along with typecheck, frontend syntax validation, and whitespace validation. The legacy test suite still needs its old single-submission fixtures migrated to cycle/version requests.
...
## 2026-09-13 — closing the page ends the review; thinking limits with hard aborts

Two reported experience problems fixed. (1) Stale reviews: closing the browser tab did nothing (reviews lived until their launcher process ended, by design), so a launch for a new commit reused the old review with its old comments. Now every open page posts `POST /api/beacon` every 10s; the launcher's sweeper tears the review down (stop server, remove lockfile, and for the CLI process exit) once no page has beaconed within a 90s grace window (env-tunable `SIDEYE_SWEEP_MS` / `SIDEYE_HEARTBEAT_GRACE_MS` for tests). Lockfile reuse also requires a matching review target: a different-commit launch replaces the running review — in-process (plugin) reviews stop cleanly, CLI-owned reviews get SIGTERM, and a foreign plugin lock is taken over and reaped by the owner's own ownership-checking sweeper. The plugin path stops the in-process server only (opencode itself is never killed); the CLI path exits the process.

(2) Thinking limits and indication. Each analysis batch prompt now races a 3-minute limit and each plan attempt a 10-minute limit (`SIDEYE_ANALYSIS_TIMEOUT_MS` / `SIDEYE_PLAN_TIMEOUT_MS`). On expiry Sideye stops waiting and aborts the session agent loop via v2 `session.abort`, so the model actually stops. The SDK folds an aborted fetch into `result.error` (name `TimeoutError`, but JSON.stringify loses it) rather than throwing — the check reads `result.error.name`. Analysis failures are now retryable from the browser via a new control-tier `POST /api/analysis/retry`; plans already had retry. The TUI gets best-effort toasts at analysis/plan start, completion, and failure; the browser shows the spinner animation in the planning and agent-working action-bar states (analysis already had one). Verified: full suite 137 pass; typecheck, `node --check`, `git diff --check` clean; throwaway stub-server + Playwright checks for the retry button flow, planning/working spinners, beacon POST, and zero right-edge overflow at desktop/390px/320px.

## 2026-09-14 — live plan-mirror history failure fixed correctly

A Redeye review disproved T32's assumption that `ignored: true` made an assistant text part presentation-only. The approved fix prompt reached the correct session, but the next assistant message failed at zero tokens with `Invalid prompt: The messages do not match the ModelMessage[] schema`; no fix tools or edits ran. OpenCode 1.18.30 source confirms model-history conversion filters ignored **user** text but unconditionally includes assistant text, while the event stream emitted `session.error` without the `session.idle` Sideye was waiting for.

Plan Markdown now attaches to the successful plan response's parent user message as ignored text, preserving TUI display while keeping it out of model replay. The fix event wait now terminates on a matching `session.error`, stores the nested provider message, and broadcasts the failure immediately instead of timing out as a stall. Regression tests assert both the parent-user part target and the live failure's error-event path. Verification: focused plan/fix suites 16 pass; full suite 138 pass; `bun run typecheck` and `git diff --check` clean. The already-corrupted Redeye review was not modified and must be relaunched after OpenCode reloads the plugin.

## 2026-09-14 — TUI-safe background failures and headless tests

The `broken-sideye.png` screenshot showed Bun's timeout source excerpt and stack trace painted below/over the OpenCode alternate screen. The analysis timeout itself behaved correctly, but `launch.ts` caught the already-recorded background failure and called `console.error`; round-capture and analysis-retry routes had the same latent path, and plan-mirror failure used `console.warn`. Because the plugin runs in OpenCode's process, those writes bypass its renderer. A shared `startAnalysis` wrapper now contains rejected background analyses after `runAnalysis` has set failed state, broadcast SSE, and sent a toast; all three callers use it. Plan-mirror failure now uses a toast. Production shared/plugin code has no direct console output; only the standalone CLI does.

The several “This site can’t be reached” tabs during tests were also real test leakage: two plugin launches and two CLI e2e launches invoked the production OS browser opener, then stopped their ephemeral servers. `SIDEYE_NO_OPEN_BROWSER=1` now suppresses the opener, plugin tests scope it around tool execution, and CLI test children receive it explicitly. Normal launches are unchanged, and the README documents the headless option. Added background-analysis containment coverage. Verification: focused analysis/plan/plugin tests 21 pass; focused plugin/CLI tests 7 pass; full suite 139 pass with no background stack output; `bun run typecheck` and `git diff --check` clean.

## 2026-09-14 — fix completion detected via session-status polling

A live Redeye review made its approved fixes (files changed on disk), but Sideye stayed on "Agent working…": OpenCode 1.18.30's event stream emitted no matching `session.idle`, and the `GET /session/{id}/message` list endpoint returns 400 for any session containing a `json_schema`-formatted prompt (`Expected OutputFormatJsonSchema`), so the structured status report could not be read after the fact either. `waitForSessionOutcome` now races three signals: the existing terminal-event wait, a 1s poll of `/session/status` (completion when the session leaves `busy`/`retry`), and the same 10-minute stall budget. The latest assistant `message.updated` event captures the structured report directly, removing the message-list dependency. Regression test: stub omits the idle event and reports non-busy via status polling → statuses still land. Verification: focused fix suite 8 pass; full suite 140 pass; typecheck and `git diff --check` clean.

Also diagnosed a live analysis failure on the Redeye review: the model spent its full 3-minute analysis budget gathering context (`read` on route/schema/lib files) and composing findings, then Sideye's hard abort fired (`MessageAbortedError`). Not a provider outage — the limit was simply too tight for real diffs. The default `SIDEYE_ANALYSIS_TIMEOUT_MS` is now 10 minutes per batch, matching the plan limit; env override and retry behavior unchanged. Decision entry appended.

## 2026-09-16 — live progress review fixes
