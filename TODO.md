...
- [ ] `T28` Give post-submit comments a channel to the agent (round 2+): either allow re-submitting newly added comments as a follow-up payload or inject them into an in-flight/next fix prompt; re-submit is currently blocked by the one-submission guard (LLD §11.6) — agent, complexity: complex
